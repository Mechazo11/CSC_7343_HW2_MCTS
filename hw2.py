...
* Action - a (i,j) position on the game board, lets imagine this for now
    ...
    * game_state = None, 0 for draw and 1 for a player win, used to determine if a node is terminal
    ...
    def find_action(self):
        ...
        * Useful notes on UCB https://www.youtube.com/watch?v=UXW2yZndl7U&ab_channel=JohnLevine
        ...
        ucb_scores = self.Q + cpuct * P_sa * (math.sqrt(sum__Nsb)/(1 + self.N)) # 1D numpy array of UCB scores
    ...
    def print_node_stats(self):
        print()
        # print(f"id: {self.id}")
        # print(f"move (x,y): {self.action_taken[0], self.action_taken[1]}")
        print(f"Number of visits: {self.N}")
        print()
...
class MCTS(MCTSBase):
    ...
    def get_visit_count(self, state):
        ...
        print(f"Number of child nodes: {len(root_childs)}")

        for chd in root_childs:
            x,y = chd.move_tuple[0],chd.move_tuple[1]
            # TODO a function to check for invalid move
            #* If invalid move, set that position 0
            
            #* else, add count
            self.move_count_matrix[x,y]+= chd.N
            
            # Do the same for all the "child nodes" of this child
            ...
            if not chd.children_nodes:
                for chdinchd in chd.children_nodes:
                    ...
                    #* check for invalid move
                    self.move_count_matrix[x_in,y_in]+= chdinchd.N
            else:
                print(f"This child node is a leaf node")

        return self.move_count_matrix

    #* Helper functions
    
    # def add_node_to_tree_graph(self,mctsNode):
    #     """
    #     Helper function that adds mctsNode into the python list and updates unique id counter and number of node count
    #     """
    #     # Append mctsNode's board state in the tree_graph_boardstates list
    #     self.tree_graph_boardstates.append(mctsNode.boardState)
        
    #     # Append this node to the self.tree_graph, 
    #     self.tree_graph.append(mctsNode)
        
    #     # Increment unique ids and node count
    #     # self.node_count = self.node_count + 1 # Redundant
    ...
    def search_for_matching_node(self, standardState):
        ...
        # # Perform element-wise comparison, both must be numpy arrays
        # matches = [np.array_equal(standardState, state) for state in self.tree_graph_boardstates]

        # # Find the index of the matching board state
        # index = next((i for i, match in enumerate(matches) if match), -1)
